...
public interface CompletedRide {
	...
	default double getPrice() {
		if (getWaitTime() < 25) {
			return (double) 2.5 * getRequest().getRideTime();
		}
		else if (getWaitTime() <= 49) {
			return (double)  2.0 * getRequest().getRideTime();
		}
		else if (getWaitTime() <= 99) {
			return (double) getRequest().getRideTime();
		}
		else {
			return (double) 0.5 * getRequest().getRideTime();
		}
	}
	...
	default double getCost() {
		return (double) (getRequest().getRideTime() * 0.5) + (getWaitTime() * 0.1);
	}
}